...
const Footer = () => {
    ...
    const renderIcon = useCallback((element: number) => {
        switch (element) {
            ...
            case 1:
                return <InstagramLogo size={20} color="currentColor" />;
            case 2:
            ...
            <main className="w-full lg:pt-28 lg:pb-12 pt-20 pb-12 px-6 grid md:grid-cols-3 lg:gap-8 md:gap-5 gap-8 lg:px-32">
                <div className="flex flex-col gap-2">
                    <Link to={`/`} className="font-extrabold flex items-center relative md:text-3xl text-2xl">
                        <Text as="span" className="text-amber-500 absolute -top-5 md:left-14 left-3">
                            <Barbell size={35} color="currentColor" weight="fill" />
                        </Text>
                        <Text as="span" className="text-white">Body</Text>
                        <Text as="span" className="bg-gradient-to-r from-amber-500 to-red-500 bg-clip-text text-transparent">Center</Text>
                    </Link>
                    <Text as="p" className="text-zinc-400 text-justify">{FooterTexts.underLogoText}</Text>
                </div>

                {/* Quick Links  */}
                <div className="flex flex-col md:items-center md:mt-0 gap-4">
                ...
                {/* Quick contacts  */}
                <div className="flex flex-col md:mt- gap-6">
                    ...
                    <ul className="flex flex-col gap-4">
                        {
                            FooterTexts.contacts.names.map((name, index) => (
                                <List className="text-zinc-400 flex items-start gap-2" key={index}>
                                    <Text as="span" className="text-amber-500 mt-0.5">{renderIcon(index)}</Text>
